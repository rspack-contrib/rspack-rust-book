...
Check out the `examples/plugin/build.js` file in the [rspack-binding-template](https://github.com/rspack-contrib/rspack-binding-template/blob/main/examples/plugin/build.js) repository. We've already created the `MyBannerPlugin` wrapper in the previous section. So we can use it in the rspack configuration.
...
You can now run `node examples/plugin/build.js` to see the plugin in action. This is also the same command as the [Verify Setup](./setup.md). But now you have the knowledge of what is happening behind the scene.
